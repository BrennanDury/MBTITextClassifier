...
def plot_tree(model, dim, vocab, depth):
    ...
    plt.savefig('Models/' + str(n) + 'n_' + str(depth) + 'd/' + dim + '_' + 'tree.png', bbox_inches='tight')
...
def equal_labels(bag, dim):
    if (dim != 'ptype'):
        ...
        even = pd.DataFrame()
        for ptype in types:
            even = even.append(bag[bag['ptype'] == ptype].head(smallest_type))
        return even
...
def model_dim(bag, vocab, dim, depth):
    ...
    features = bag
    labels = features[dim].astype(str)
    features = features.drop(columns=['Unnamed: 0', 'ptype', 'E/I', 'S/N', 'T/F', 'J/P'])
    ...
    if dim != 'ptype':
        dim = dim[0] + '_' + dim[2]
    with open('Models/' + str(n) + 'n_' + str(depth) + 'd/' + dim + '_' + 'model.pkl', 'wb') as file:
        pickle.dump(model, file)
    plot_tree(model, dim, vocab, depth)
    with open('Models/' + str(n) + 'n_' + str(depth) + 'd/' + dim + '_' + 'results.json', 'w') as file:
